...
public class ValidationQuestionnaire extends HttpServlet {
	...
	public void doGet( HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException{
		...
		ArrayList questionnaires_effectues = (ArrayList) session.getAttribute("tableau_de_questionnaires");
		boolean deja_fait;
		
		
		
		if( questionnaires_effectues == null || questionnaires_effectues.contains(questionnaire.getId()) == false ){
			
			//Encore fait aucun questionnaire
			if(questionnaires_effectues == null){
					ArrayList nouveau_tableau_de_questionnaire = new ArrayList();
					nouveau_tableau_de_questionnaire.add(questionnaire.getId());
					session.setAttribute("tableau_de_questionnaires", nouveau_tableau_de_questionnaire);
			}
			//Deja fait d'autres questionnaires
			else{
				questionnaires_effectues.add(questionnaire.getId());
				session.setAttribute("tableau_de_questionnaires", questionnaires_effectues);
			}
